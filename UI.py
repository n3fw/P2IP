...
class UI():
    ...
    def connexionWindow(self):
        ...
        self.root.title("EcoLend - Connexion")
        self.root.configure(bg="white")

        def connect():
            email = uVar.get().strip()
            pwd = pVar.get().strip()

            if not email or not pwd:
                messagebox.showwarning("Formulaire incomplet", "Veuillez entrer votre email et mot de passe.")
                return

            # VRAIE VÉRIFICATION DANS LA BASE DE DONNÉES
            query = "SELECT id FROM users WHERE email = %s AND password_hash = %s"
            result = self.db.c_query(query, (email, pwd), ret=True)

            if result: # Si le compte a été trouvé
                self.current_user_id = result[0][0] # On stocke le VRAI id de l'utilisateur connecté
                self.action_id = 0 # Envoie vers l'accueil via le main
                self.root.quit() 
            else:
                # Si le mot de passe est faux ou le compte inexistant
                messagebox.showerror("Erreur d'authentification", "Email ou mot de passe incorrect.")
            
        def create():
            self.action_id = 1 # Envoie vers la création de compte
            self.root.quit() 

        tText = tk.Label(self.root, font = (self.font, 20), text = "Connexion à votre compte", bg = "white", fg = "#17E63C")
        tText.place_configure(relx = 0.19, rely = 0.3)

        uText = tk.Label(self.root, font = (self.font, 10), text = "Addresse email", bg = "white", fg = "black")
        uText.place_configure(relx = 0.25, rely = 0.42)

        pText = tk.Label(self.root, font = (self.font, 10), text = "Mot de passe", bg = "white", fg = "black")
        pText.place_configure(relx = 0.25, rely = 0.56)

        uVar = tk.StringVar(self.root)
        uEntry = tk.Entry(self.root, width = 40, textvariable = uVar, bg = "#DBD9D9", fg = "black")
        uEntry.place_configure(relx = 0.255, rely = 0.48)

        pVar = tk.StringVar(self.root)
        ...
        ConnectButton = tk.Button(self.root, text = "Connexion", command=connect)
    ...
    def creationWindow(self):
        ...
        self.root.title("EcoLend - Créer un compte")
        self.root.configure(bg="white")
        ...
        def valider():
            ...
            try:
                # 1. On insère le nouvel utilisateur dans la BDD
                query = "INSERT INTO users (firstname, lastname, email, password_hash, city) VALUES (%s, %s, %s, %s, %s)"
                self.db.c_query(query, (prenom, nom, email, pwd, ville), ret=False)
                
                # 2. On récupère son nouvel ID fraîchement créé pour le connecter
                get_id_query = "SELECT id FROM users WHERE email = %s"
                nouveau_user = self.db.c_query(get_id_query, (email,), ret=True)
                
                if nouveau_user:
                    self.current_user_id = nouveau_user[0][0] # Mise à jour de l'ID actif !

                messagebox.showinfo("Succès", f"Bienvenue {prenom} ! Ton compte a été créé.")
                self.action_id = 2 # On l'envoie sur l'accueil
                self.root.quit()
                
            except Exception as e:
                errLabel.config(text="Erreur : Cet email est peut-être déjà utilisé.")

        tk.Label(self.root, font=(self.font, 18), text="Créer un compte", bg="white", fg="#17E63C").place(relx=0.22, rely=0.02)

        tk.Label(self.root, font=(self.font, 10), text="Prénom *", bg="white", fg="black").place(relx=0.25, rely=0.10)
        prenomVar = tk.StringVar(self.root)
        tk.Entry(self.root, width=40, textvariable=prenomVar, bg="#DBD9D9", fg="black").place(relx=0.255, rely=0.16)

        tk.Label(self.root, font=(self.font, 10), text="Nom *", bg="white", fg="black").place(relx=0.25, rely=0.24)
        nomVar = tk.StringVar(self.root)
        tk.Entry(self.root, width=40, textvariable=nomVar, bg="#DBD9D9", fg="black").place(relx=0.255, rely=0.30)

        tk.Label(self.root, font=(self.font, 10), text="Adresse email *", bg="white", fg="black").place(relx=0.25, rely=0.38)
        emailVar = tk.StringVar(self.root)
        tk.Entry(self.root, width=40, textvariable=emailVar, bg="#DBD9D9", fg="black").place(relx=0.255, rely=0.44)

        tk.Label(self.root, font=(self.font, 10), text="Mot de passe *", bg="white", fg="black").place(relx=0.25, rely=0.52)
        pwdVar = tk.StringVar(self.root)
        tk.Entry(self.root, width=40, textvariable=pwdVar, show="•", bg="#DBD9D9", fg="black").place(relx=0.255, rely=0.58)

        tk.Label(self.root, font=(self.font, 10), text="Confirmer le mot de passe *", bg="white", fg="black").place(relx=0.25, rely=0.66)
        pwd2Var = tk.StringVar(self.root)
        tk.Entry(self.root, width=40, textvariable=pwd2Var, show="•", bg="#DBD9D9", fg="black").place(relx=0.255, rely=0.72)

        tk.Label(self.root, font=(self.font, 9), text="Ville (optionnel)", bg="white", fg="gray").place(relx=0.25, rely=0.80)
        villeVar = tk.StringVar(self.root)
        tk.Entry(self.root, width=20, textvariable=villeVar, bg="#DBD9D9", fg="black").place(relx=0.255, rely=0.86)

        errLabel = tk.Label(self.root, font=(self.font, 9), text="", bg="white", fg="red")
    ...
    def accueilWindow(self):
        """
        fenêtre d'acceuil qui affiche les annonces actuelles, sur laquelle est branché le programme après connexion / creation de compte
        """
        self.root = tk.Tk()
        ...
        def go_to_detail(annonce_id):
            self.selected_annonce_id = annonce_id
            self.action_id = 4 
            self.root.quit() # <-- CORRECTION : Remplacer destroy() par quit()

        if annonces_db:
            for annonce in annonces_db:
                card = tk.Frame(body, bg="#FFFFFF", padx=10, pady=8, bd=1, relief="flat")
                card.pack(fill="x", pady=4)
                
                texte = f"{annonce[1]} (Catégorie: {annonce[3]}) - à {annonce[4]}"
                tk.Label(card, text=texte, font=("Helvetica", 11), bg="#FFFFFF").pack(side="left")
                
                # Le bouton qui envoie l'ID (annonce[0]) vers la fenêtre de détails
                tk.Button(card, text="Voir détails", bg="#52B788", fg="#FFFFFF", relief="flat", cursor="hand2",
                          command=lambda a_id=annonce[0]: go_to_detail(a_id)).pack(side="right", padx=10)
        else:
    ...
    def listeAnnoncesWindow(self):
        ...
        query = """
            SELECT L.id, L.title, C.name, U.city, L.tool_condition, L.description
            FROM listings L
            JOIN categories C ON L.category_id = C.id
            JOIN users U ON L.user_id = U.id
            WHERE L.status = 'active'
        ...
        if annonces:
            for a in annonces:
                ...
                # Note le changement : les index ont décalé de +1 à cause de l'ID en a[0]
                details_text = f"📍 {a[3]}  •  🏷️ {a[2]}  •  État : {a[4]}"
                tk.Label(card, text=details_text, font=("Helvetica", 9), fg="#6B7280", bg="#FFFFFF").pack(anchor="w", pady=4)
                tk.Label(card, text=a[5][:100] + "..." if len(a[5]) > 100 else a[5], font=("Helvetica", 9), fg="#6B7280", bg="#FFFFFF", wraplength=600, justify="left").pack(anchor="w")
                ...
                tk.Button(card, text="Voir le détail →", bg="#2D6A4F", fg="#FFFFFF", relief="flat", font=("Helvetica", 10, "bold"), cursor="hand2", padx=12, pady=4, 
                          command=lambda a_id=a[0]: go_to_detail_liste(a_id)).pack(anchor="e", pady=5)
        else:
    ...
    def profilWindow(self):
        ...
        def go_to_creation():
            self.action_id = 8 # 8 = ID de la fenêtre création d'annonce
            self.root.quit()
        ...
        def display_annonces_tab():
            tk.Button(body_frame, text="+ Publier une nouvelle annonce", bg="#52B788", fg="#FFFFFF", 
                      ...
                      command=go_to_creation).pack(anchor="e", pady=(0, 12))
    ...
    def detailAnnonceWindow(self):
        ...
        query = """
            ...
            WHERE L.id = %s
        """
        result = self.db.c_query(query, (self.selected_annonce_id,), ret=True)
        if not result:
            self.action_id = 2 # Retour sécurité si l'annonce n'existe plus
            self.root.destroy()
            return
        ...
        def go_back():
            self.action_id = 2 # Retour à l'accueil
            self.root.destroy()
    ...
    def creationAnnonceWindow(self):
        self.root = tk.Tk()
        self.action_id = None # Sécurité anti-boucle
        self.root.geometry("600x650")
        self.root.title("EcoLend - Publier une annonce")
        self.root.configure(bg="#F4F6F8")

        # 1. Récupérer les catégories depuis la BDD (pour le menu déroulant)
        ...
        def valider():
            ...
            try:
                ...
                """
                self.db.c_query(query, (self.current_user_id, cat_id, titre, description, etat), ret=False)
